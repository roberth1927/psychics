...
class cron_payout_record_process extends Command
{
    ...
    public function handle()
    {
        ...
        $users = User::where(['role_id'=>1,'email_validated'=>1, 'account_status'=> 'ACTIVE'])->get();

        foreach ($users as $user) {

            //S_REFUND 
            echo 'Psychic with id: '.$user->id.' status: Processing'; 
            $current_balance_net_withoutrefund = $user->psychic_credit_logs()  
                                        ->where('action','<>','REFUND')                                      
                                        ->whereBetween('created_at',[$start,$compareEndDay])
                                        ->sum('credits');

            $current_balance_net_commission = $user->psychic_credit_logs()  
                                        ->where('action','<>','REFUND')                                      
                                        ->whereBetween('created_at',[$start,$compareEndDay])
                                        ->sum('credits_commission');                            
            ...
            $current_balance_net_refunded_commission = $user->psychic_credit_logs()
                                        ->where('action','=','REFUND')                                      
                                        ->whereBetween('created_at',[$start,$compareEndDay])
                                        ->sum('credits_commission');                            
            ...
            $current_balance = $current_balance_net_commission - $current_balance_net_refunded_commission;
