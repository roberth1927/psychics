...
class Day10ClientEmail extends Command
{
    ...
    public function handle()
    {
        ...
        if(!empty($users)){
            ...
            $users2 = User::selectRaw('user.*')->join('user_profile', 'user.id', '=', 'user_profile.user_id')
            ->where('email_validated', 1)
            ->where('fraud', 0)
            ->where('user.role_id', '=', WhiteLabel::roleId('Psychic'))
            ->where('user.test_user', 0)//->where('user.id', '=', 12634)->orWhere('user.id', '=', 4458)->orWhere('user.id', '=', 2096)
