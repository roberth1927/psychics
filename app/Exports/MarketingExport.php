...
class MarketingExport implements FromArray, WithMapping, WithHeadings, WithEvents
{
    ...
    public function map($data): array
    {
        ...
        $rows[] = \array_merge([''], ['(fbclid) paid social LEADS'], \array_map(function($v){return $v == 0 ? "0" : $v;}, $data['CLID']['fbclidLead']), [\array_sum(\array_map(function($v){return $v == 0 ? "0" : $v;}, $data['CLID']['fbclidLead']))]);
        $rows[] = \array_merge([''], ['(fbclid) paid social Revenue'], \array_map($format, $data['CLID']['fbclidNet']), [$format(\array_sum($data['CLID']['fbclidNet']))]);
        $rows[] = \array_merge([''], ['(gclid) paid search LEADS'], \array_map(function($v){return $v == 0 ? "0" : $v;}, $data['CLID']['gclidLead']), [\array_sum(\array_map(function($v){return $v == 0 ? "0" : $v;}, $data['CLID']['gclidLead']))]);
        $rows[] = \array_merge([''], ['(gclid) paid search Revenue'], \array_map($format, $data['CLID']['gclidNet']), [$format(\array_sum($data['CLID']['gclidNet']))]);
