...
trait ServiceTrail
{
    ...
    public function psychicService()
    {
        ...
        $query = User::selectRaw('date(user_1_1_chat_request.created_at) as date, count(.user_1_1_chat_request.id) as cant')->join('user_1_1_chat_request', 'user_1_1_chat_request.model_id', '=', 'user.id')->
            where('role_id', WhiteLabel::roleId('Psychic'))->where('fraud', 0)->where('test_user', 0)->where('email_validated', 1)
            ->where('user_1_1_chat_request.state', 'COMPLETE');
        ...
        //     where('role_id', WhiteLabel::roleId('Psychic'))->where('fraud', 0)->where('test_user', 0)->where('email_validated', 1)
        //     ->where('user_1_1_chat_request.state', 'COMPLETE');
        // $psychics = User::where('role_id', WhiteLabel::roleId('Psychic'))->where('fraud', 0)->where('test_user', 0)->where('email_validated', 1)->count();
        ...
        $avg_video = $collection->count() == 0 ? 0 : \round($collection->sum(function ($item) { //Average number of video chat sessions completed per Psychic
            return $item['cant'];
        }) / $collection->count(), 2);
        ...
        $collection = chat::selectRaw('date(chat.created_at) as date, count(chat.id) as cant')->join('user', 'user.id', '=', 'chat.receiver_id')->where('user.role_id', WhiteLabel::roleId('Psychic'))->groupBy('date')->get();
