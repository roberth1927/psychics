...
trait SearchUser
{
    ...
    {       
        ...
        if ($request->has('has_email_validated')) {
            $query->where('email_validated', 1);
        }
        ...
        if($filter = $request->get('filter')){
            if($filter == 'no-validate')
            $query->whereRaw('(user_mobile_num.validated IS NULL OR user_mobile_num.validated = 0)');
        }
