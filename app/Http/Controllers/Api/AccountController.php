...
class AccountController extends Controller
{
    ...
    public function saveProfile(Request $request)
    {
       ...
       $validator = $request->validate([
        ...
        $validator =  $request->validate([  
            ...
            'profile.description' => "min:200|max:400|regex:/^[a-z\d,.!'’?\-_\s]+$/i",
        ...
        if((is_null($user_profile->tagline)) || (preg_match("([^A-Za-z0-9 ,.!'?])",$user_profile->display_name))){
            $user_profile->display_name = $request->input('profile.display_name');
            $user_profile->profile_link = WebUtils::seoUrl($request->input('profile.display_name'));
        }
    ...
    if($request->tab == 3) {
        ...
        $user_profile->get_featured = $request->input('profile.get_featured');
        ...
        $specialtiesIds = [];
        foreach ($request->input('specialties') as $key => $sp){
            $specialtiesIds[] = $sp['id'];
        }
        ...
        $toolsIds = [];
        foreach ($request->input('tools') as $key => $tool){
            $toolsIds[] = $tool['id'];
        }
        $user->tools()->sync($toolsIds);
        $user->save();

       
        $stylesIds = [];
        foreach ($request->input('styles') as $key => $style){
            $stylesIds[] = $style['id'];
        }
        $user->styles()->sync($stylesIds);
        $user->save();
       ...
       // dd($credentialsInput);
        foreach ($credentialsInput as $key => $credential)
        {
            if(count($credentials) > $key){
                $credentials[$key]->institution_name = $credential['institution_name'];
                $credentials[$key]->from_year = $credential['from_year'];
                $credentials[$key]->to_year = $credential['to_year'];
                $credentials[$key]->degree = $credential['degree'];
                $credentials[$key]->area_of_study = $credential['area_of_study'];
                $credentials[$key]->description = $credential['description'];
                $credentials[$key]->save();
            }else{
                $newcredential = new UserCredentials();
                $newcredential->institution_name = $credential['institution_name'];
                $newcredential->from_year = $credential['from_year'];
                $newcredential->to_year = $credential['to_year'];
                $newcredential->degree = $credential['degree'];
                $newcredential->area_of_study = $credential['area_of_study'];
                $newcredential->description = $credential['description'];
                $newcredential->user()->associate($user);
                $newcredential->save();
            }
        ...
        $worksInput = $request->input('works');
        $works = $user->works()->get();
       // dd($worksInput);
        foreach ($worksInput as $key => $work)
        {
            if(count($works) > $key){
                $works[$key]->position = $work['position'];
                $works[$key]->from_year = $work['from_year'];
                $works[$key]->to_year = $work['to_year'];
                $works[$key]->company = $work['company'];
                $works[$key]->description = $work['description'];
                $works[$key]->save();
            }else{
                $newwork = new UserWorks();
                $newwork->position = $work['position'];
                $newwork->from_year = $work['from_year'];
                $newwork->to_year = $work['to_year'];
                $newwork->company = $work['company'];
                $newwork->description = $work['description'];
                $newwork->user()->associate($user);
                $newwork->save();
            }
    ...
    {
        $lang = $user->languages()->find($language->id);
        $lang->delete();
       
    }

    
    foreach ($languagesInput as $key => $language)
    {

            $newlanguage = new UserLanguages();
            $newlanguage->languages_id = $language['languages_id'];
            $newlanguage->user()->associate($user);
            $newlanguage->save();
    ...
    //     $user->UserLanguages()->sync($languagesInput );
    //     $user->save();
    ...
    //             $languages[$key]->languages_id = $language['languages_id'];
    //             // $languages->Languages()->associate($language);
    //             $languages[$key]->save();
    //         }else{
    //             $newlanguage = new UserLanguages();
    //             $newlanguage->languages_id = $language['languages_id'];
    ...
    public function uploadProfileVideo(Request $request)
    {
      ...
      /*  $this->validate($request, [
        ...
        if ($request->hasFile('file')) {
            $profile->video_status_view = 1;
            $profile->save();
            UploadUtils::uploadProfileVideo($request, 'file', $profile);
        }


        return ApiUtils::response_success(['message'=> 'Video saved', 'video_path'=> $profile->intro_video_path], 200);
    ...
    public function save_card_payment(Request $request)
    {
        ...
        if(($user->promo()>0)&&($user->referred_by_user))
        $promo = $request->credits<50 ? 5 : 10;
        else $promo = $user->promo();
        ...
        if ($code == "100") {
            ...
            if ($code == "100") {
                ...
                if(isset($promoBuyCredit)){
                    ...
                    if(strtoupper($promoBuyCredit->code) == "PAULA")
                    $promo = 0;
                }
                ...
                if ($isFirstPurchase) {
                  if ($user->referred_by_user) {
                    ...
                    $user_credit_log->action = '10FROMAFRIEND';
                    $user->credits += 10;

                    $referUser = User::find($user->referred_by_user);
                    $user_credit_logr = new UserCreditLog();
                    $user_credit_logr->site_id = WhiteLabel::site_id();
                    $user_credit_logr->credits_old = $referUser->credits;
                    $user_credit_logr->credits = 10;
                    $user_credit_logr->action = 'ReferralRewardEarned';
                    if ($referUser->role_id == 2) {
                      $referUser->user_credit_logs()->save($user_credit_logr);
                    } else {
                      $user_credit_logr->credits_commission = 10;
                      $user_credit_logr->user_id = $user->referred_by_user;
                      $user_credit_logr->psychic_id = $user->referred_by_user;
                      $user_credit_logr->save();
                    }
                    $referUser->credits += 10;
                    $referUser->save();
                    $user->user_credit_logs()->save($user_credit_log);
                  }
                }
                ...
                //Todo send Email
                if($user->gclid !== null){
                    $data   = ['gclid'=> $user->gclid, 'signup'=> $user->created_at, 'first_purchase'=> $isFirstPurchase , 'amount'=> $amount, 'user_id'=> $user->id];  
                    WebUtils::sendGclid($data);
                }
    ...
    public function save_card_subscribe(Request $request)
    {
        ...
        if ($code == "100") {
            ...
            if ($code == "100") {
                ...
                return ApiUtils::response_success('You have subscribed successfully. Thank you for using Psychics1on1™.', Response::HTTP_CREATED);
    ...
    public function subscribe(Request $request)
    {
        ...
        if ($code == "100") {
            ...
            Log::info('From Accountcontroller subscribe()  -- User credit old: $'.$user->credits. ' Amount: $'.$amount.' User ID:'.$user->id);
          
         
            $user->credits -= $amount;
            $user->save();

            Log::info('From Accountcontroller subscribe() -- User credit new: $'.$user->credits.' User ID:'.$user->id);
    ...
    public function save_deposit_account(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'account_number' => 'required|min:7|max:17',
            'address_country' => 'required',
        ]);
    ...
    public function validate_apple_session(Request $request){
        ...
        if(isset($result_promo->error)){           
           ...
           if( "https" == parse_url($validation_url, PHP_URL_SCHEME) && substr( parse_url($validation_url, PHP_URL_HOST), -10 )  == ".apple.com" ){
            ...
            $ch = curl_init();        
            $data = '{"merchantIdentifier":"merchant.com.psychics1on1", "domainName":"'.$_SERVER["HTTP_HOST"].'", "displayName":"Psychics1on1"}';
