...
//      Log::debug(print_r($user_1_1_chat_request,true));
    ...
    public function removeNotifications(Request $request){
        ...
        if(($request->type == 'to_online_announcement_cosmic')&&(date('Y-m-d')>'2021-06-04')){
          

            $user->online_announcement_cosmic = 1;  
            $user->save();         
            
        } 

        if($request->type == 'to_online_announcement_cosmic_refer'){
          
            $user->online_announcement_cosmic_refer = 1;  
            $user->save();         
            
        } 

        
        if($request->type == 'to_online_update_payout_method'){

            $user->online_payout_method = 1;  
            $user->save();         
