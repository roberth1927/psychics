...
class AdminController extends Controller
{
    ...
    protected function getUsersService(Request $request)
    {
        ...
        foreach (['Psychic', 'User'] as $type) {

            $values = $type == 'Psychic' ? $this->psychicService() : $this->clientService();
    ...
    protected function updateUser($id, Request $request)
    {
        ...
        if (isset($input['role_id']) && $item->role_id != $input['role_id']) {
            if (WhiteLabel::roleId('Psychic') == $input['role_id'] && !UserService::where('user_id', $item->id)->count()) {
                for ($i = 1; $i <= 3; $i++) {
        ...
        if (isset($input['phone_numbers']) && $input['phone_numbers']['number']) {
            ...
            if ($phone->number != $input['phone_numbers']['number']) {
                ...
                $phone->validated = 0;
                $phone->active = 0;
                //Remove Sending Verification Code on Phone Number change 
                /* $code = rand(1000, 9999);
                $phone->verification_code = $code;
                TwilioUtils::send_verification_sms($phone, $item);*/
            }
    ...
    protected function updateCategory($id, Request $request)
    {
        ...
        if ($color = $request->get('color')) {
            $item->color = $color;
            $item->save();
        } else if ($image = $request->get('image')) {
            if ($img = $item->image) {
                $img->delete();
            }
            ...
            $item->name = $name;
            $item->save();
        } else if ($description = $request->get('description')) {
    ...
    protected function payouts(Request $request)
    {
        ...
        $from = 'from_payout';
        $request->request->add(['no_test_user' => true,'no_fraud_user'=>true,'has_email_validated'=>true,'with_deposit_account'=>true, 'account_active'=>true]);
    ...
    protected function transactions(Request $request)
    {
        ...
        if ($id = $request->get('user')) {
            $user = User::find($id);
            if (WhiteLabel::roleId('Psychic') == $user->role_id) {
    ...
    protected function signUp(Request $request)
    {
        ...
        if (!isset($data['month'])) {
            ...
            $psychics = User::select(DB::raw('count(id) as `total`'), DB::raw('MONTH(created_at) as `interval`'))->where('email_validated', 1)->where('fraud', 0)->where('test_user', 0)
                ->where('role_id', WhiteLabel::roleId('Psychic'))->whereBetween('created_at', [$date->copy()->startOfYear(), $date->copy()->endOfYear()])->orderBy('created_at')
                ->groupBy('interval')->get();
            $users = User::select(DB::raw('count(id) as `total`'), DB::raw('MONTH(created_at) as `interval`'))->where('email_validated', 1)->where('fraud', 0)->where('test_user', 0)
            ...
            return ['psychics' => $psychics, 'users' => $users];

        } else {
            $date = Carbon::createFromDate($data['year'], $data['month']);
            $psychics = User::select(DB::raw('count(id) as `total`'), DB::raw('DAY(created_at) as `interval`'))->where('email_validated', 1)->where('fraud', 0)->where('test_user', 0)
                ->where('role_id', WhiteLabel::roleId('Psychic'))->whereBetween('created_at', [$date->copy()->startOfMonth(), $date->copy()->endOfMonth()])->orderBy('created_at')
                ->groupBy('interval')->get();
            $users = User::select(DB::raw('count(id) as `total`'), DB::raw('DAY(created_at) as `interval`'))->where('email_validated', 1)->where('fraud', 0)->where('test_user', 0)
    ...
    protected function userStatus(Request $request)
    {
        ...
        if (!isset($data['month']) && isset($data['year'])) {
            ...
            $psychics_inactive = User::select(DB::raw('count(id) as `total`'), DB::raw('MONTH(created_at) as `interval`'))->where('role_id', WhiteLabel::roleId('Psychic'))->where('email_validated', 0)
            ...
            $psychics_active = User::select(DB::raw('count(id) as `total`'), DB::raw('MONTH(created_at) as `interval`'))->where('role_id', WhiteLabel::roleId('Psychic'))->where('email_validated', 1)
                ->where('fraud', 0)->where('test_user', 0)->whereBetween('created_at', [$date->copy()->startOfYear(), $date->copy()->endOfYear()])->orderBy('created_at')->groupBy('interval')->get();
            ...
            $psychic_hidden = User::select(DB::raw('count(id) as `total`'), DB::raw('MONTH(created_at) as `interval`'))->where('role_id', WhiteLabel::roleId('Psychic'))->where('email_validated', 1)->where('account_status', 'INACTIVE')
                ->where('fraud', 0)->where('test_user', 0)->whereBetween('created_at', [$date->copy()->startOfYear(), $date->copy()->endOfYear()])->orderBy('created_at')->groupBy('interval')->get();
            ...
            $psychics_fraud = User::select(DB::raw('count(id) as `total`'), DB::raw('MONTH(created_at) as `interval`'))->where('role_id', WhiteLabel::roleId('Psychic'))
                ->where('test_user', 1)->whereBetween('created_at', [$date->copy()->startOfYear(), $date->copy()->endOfYear()])->orderBy('created_at')->groupBy('interval')->get();
            ...
            $psychics_inactive = User::where('role_id', WhiteLabel::roleId('Psychic'))->where('email_validated', 0)
                ->where('fraud', 0)->where('test_user', 0)->count();

            $psychics_active = User::where('role_id', WhiteLabel::roleId('Psychic'))->where('email_validated', 1)
                ->where('fraud', 0)->where('test_user', 0)->count();
            ...
            $psychic_hidden = User::where('role_id', WhiteLabel::roleId('Psychic'))->where('email_validated', 1)->where('account_status', 'INACTIVE')
                ->where('fraud', 0)->where('test_user', 0)->count();
    ...
    public function getPayoutsPendingAmount(Request $request){
      
        $request->request->add(['with_deposit_account'=>true]);
        if($request->type == 'pendingTotalAmount'){
           
            $query = DB::table('payout_log')
            ->join('user', 'user.id', '=', 'payout_log.psychic_id')
            ->where(['user.test_user'=> 0,'user.email_validated' => 1,'user.fraud' => 0,'payout_log.status'=>'Pending'])
            ->where('payout_log.payout_to_pay','<>',0)
            ->whereDate('payout_log.pay_day', '<=' ,date('Y-m-d'));
                   
          

            if($request->has('with_deposit_account')){
                if($request->has('with_no_deposit_account')){
                    // $query->leftJoin('user_biller_edata', 'user_biller_edata.user_id', '=', 'user.id')
                    //       ->where('user_biller_edata.type','=',NULL);
                    $query->where('deposit_account', 0);
                } else{
                    $query->where('deposit_account', 1);
                    //$query->join('user_biller_edata', 'user_biller_edata.user_id', '=', 'user.id');
                }            
            }

            $totalPending = $query->sum('payout_log.payout_to_pay');

            return [
            'pendingTotalAmount'=> number_format($totalPending, 2, '.', '')
             ];
        }
    return 'ok';
    }
