...
class UserServicesController extends Controller
{
    ...
    public function save(Request $request) {
        ...
        if ($request->input("free_chat") === true) {
            $request->validate([
                'user_id' => 'required',
                'user_services' => 'array',
                'user_services.*.active' => 'boolean',
                'user_services.0.rate' => 'required_if:user_services.0.active,1|numeric|min:2',
                'user_services.1.rate' => 'required_if:user_services.1.active,1|numeric',
                'user_services.2.rate' => 'required_if:user_services.2.active,1|numeric|min:5',
                'user_services.0.min_duration' => 'required|integer|min:5',
                'user_services.2.min_duration' => 'required|integer|min:5',
            ]);
        } 
        elseif (!$user->free_chat) {
            $request->validate([
                'user_id' => 'required',
                'user_services' => 'array',
                'user_services.*.active' => 'boolean',
                'user_services.0.rate' => 'required_if:user_services.0.active,1|numeric|min:1.5',
                'user_services.1.rate' => 'required_if:user_services.1.active,1|numeric|min:0.75',
