<template>
  <div>
    <v-window v-if="!rows && !filters.rating" v-model="window">
      <v-window-item
        v-if="psychics.length"
        v-for="(psychics, index) in items"
        :key="index"
        
      >
      <!-- :style="mobile ? 'min-height: 504px;' : ''" -->
        <v-row v-if="!mobile" no-gutters class="ma-0 d-flex justify-center">
          <v-col
            :class="`pa-${number == 1 ? 3 : 1} pa-md-4`"
            :cols="12 / number"
            v-for="(item, index) in psychics"
            :key="index"
          >
        ...
        <v-row v-else no-gutters class="ma-0 d-flex justify-center">
          <v-col
            :cols="12 / number"
          >
            <swiper
              :options="swiperOptions"
              ref="mySwiperListMobile"
              class="row"
            >
              <swiper-slide v-for="(item, index) in psychics" :key="index">
                <v-hover v-slot:default="{ hover }">
                  <v-card
                    v-if="!item.loading"
                    style="
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="row mt-2">
                        <div class="col" style="padding: 0px 3px !important">
                          <span v-for="(cat, index) in item.categories">
                            <img v-if="index < 4" :src="cat.path" />
                          </span>
                        </div>
      ...
      <v-row
        ...
        v-else-if="psychics.length == 0"
      >
        <v-col
          class="d-flex align-center justify-center"
          :style="{ minHeight: number == 1 ? '100px' : '430px' }"
        >
          <label style="color: #a163c1; font-size: 24px"
            >No psychics found.</label
          >
        </v-col>
      </v-row>
    </v-window>
    <v-window v-else-if="items.length && filters.rating" v-model="window">
      <v-window-item>
                ...
                <div>
                  ...
                  >
                    <div class="col-12 p-0">
                      <a v-bind:href="'/' + item.profile.profile_link" :class="item.t_cosmic>=500 ? 'counterCosmicCreditsCardCircle' :''">
                        <v-badge
                          :class="
                            $vuetify.breakpoint.xsOnly
                              ? 'user_badge_lg'
...
export default {
    ...
    window(val) {
      ...
      if (val + 1 > this.page) {
        ...
        this.$store.dispatch("psychic/getItems", this.getParams()).then(() => {
          ...
          this.items[this.items.length - 1] = this.psychics.data;

          if (this.psychics.meta.last_page-1 >= this.page) {
  ...
  computed: {
    swiper() {
      return this.$refs.mySwiperListMobile[0] ? this.$refs.mySwiperListMobile[0].$swiper : null;
    },
    ...mapGetters({
      mobile: "util/mobile",
    ...
    async loadMore() {
      ...
      await this.$store
        ...
        .then(() => {
          ...
          this.items.push(...this.psychics.data);

          if (this.psychics.meta.last_page == this.page)
    ...
    setOnlineStatus(data) {
        for (let i = 0; i < this.items.length; i++) {
          if (this.items[i].id == data.id) {
            this.items[i].online = data.status;
            break;
          }
        }
      }
  ...
  created() {
    ...
    if (!this.skeletor&&!this.psychicsProp) {
      this.$store.dispatch("psychic/perPage", this.perPage);
      ...
      this.$store.dispatch("psychic/getItems", params).then(() => {
        ...
        const items = this.psychics.data;
        ...
        if (this.rows) {
          ...
          if (this.psychics.meta.current_page < this.psychics.meta.last_page)
            this.items.push(this.getLoadingArray());
        }
  ...
  mounted() {
    if (this.psychicsProp) {
      if (this.mobile){
        let newa = [];
        newa[0] = this.psychicsProp;
        this.items = newa
      }else{
        this.items = this.psychicsProp;
      }
    }
