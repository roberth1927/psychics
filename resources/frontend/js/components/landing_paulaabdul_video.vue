  ...
  <div class="bg-white">
    <div class="pa-0">
      <v-img
        ...
        style="z-index: 1"
        :style="{cursor: mobile ? 'pointer' : ''}"
        @click="openVideo('bgImage')"
        :src="$vuetify.breakpoint.xsOnly ? 
          '/images/Banner-imgs/banner_lp_paula_video_mobile.png':
          '/images/Banner-imgs/banner_lp_paula_video.png'"
        aspect-ratio="1"
        :max-height="mobile ? 714 : 720"
        :height="mobile ? 714 : 720"
        class="align-sm-center justify-sm-center home_hero"
      >
        <div
          class="text-center main_copy_cta"
          :style="{ top: mobile ? '0' : '19%', marginTop: mobile ? '390px' : '0px' }"
        >
          <p class="main_title_lp_paula">Paula Abdul<br v-if="mobile"/> x<br v-if="!mobile"/>
            Psychics1on1™</p>
          <p class="sub_title_main_lp_paula">
            You’ll get $10 off your first<br v-if="!mobile"/> reading<br v-if="mobile"/>
            when you use promo<br v-if="!mobile"/> code PAULA
          </p>
          <div
            ...
            <img src="/images/icons/play_icon.svg"
              class="m-auto d-flex"
              @click="openVideo('button')"
              style="z-index: 1;"
              :style="{ height: mobile ? '60px' : '100px', 
                position: mobile ? 'absolute' : '',
                marginTop: mobile ? '10px !important' : '',
                cursor: 'pointer',
                right: mobile ? '5%' : '',
                top: mobile ? '0' : ''}"
            />
        <div
          class="text-left text-sm-right block_quote_h_v"
          :style="{ top: mobile ? '25px' : '40px' }"
        >
          <div style="display: grid; margin: auto;"
            ...
            />
            <p class="italic_additional_info m-0">“Check out my video!”</p>
          </div>
        </div>
      </v-img>
      <img src="/images/icons/double_chevron_icon.svg"
        class="chevron_down d-none"
    ...
    <modal-video :channel="videoChannel" :videoId="videoId" :isOpen.sync="videoIsOpen" :youtube="youtube" />
...
import ModalVideo from "./modal_video.vue"
...
export default {
  ...
  methods: {
    openVideo(from) {
      if (from=='button'&&!this.mobile) {
        console.log("eeeyyyyy if");
        this.videoIsOpen = !this.videoIsOpen 
      }
      if (from=='bgImage'&&this.mobile){
        console.log("eeeyyyyy else");
        this.videoIsOpen = !this.videoIsOpen 
      }
    },
    goToUrl(url){
...
@media (max-width: 576px){
  ...
  .main_copy_cta{
    backdrop-filter: brightness(0.5) blur(30px);
    -webkit-backdrop-filter: brightness(0.5) blur(30px);
  }
  .custom_card_rating{
    width: 89%;
  }
  .italic_additional_info{
    padding-right: 25px;
    text-align: right;
    font: italic normal 600 14px/28px Montserrat;
