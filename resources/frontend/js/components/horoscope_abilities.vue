        ...
        <v-col>
          ...
          <swiper v-else ref="mySwiper" :options="swiperOptions">
            <swiper-slide v-for="(item2, index) in items" :key="index">
              <div class="my-4 ability_card" style="height: 100px" v-for="(item, index2) in item2" :key="index2">
                <v-hover v-if="!item.loading" v-slot:default="{ hover }">
                  <a
                    :href="'/search?ability=' + item.slug"
                    style="text-decoration: none"
...
.imgHoroscopeFooterHomeD{
  background: url('/images/seo/HomeHeroImageDesktop@2x.png');
...
@media screen and (max-width: 600px) {
  ...
  .imgHoroscopeFooterHomeD{
  background: url('/images/seo/HomeHeroImageMobile@2x.png');
  min-height: 245px;
}
