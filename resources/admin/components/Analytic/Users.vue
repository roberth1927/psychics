          ...
          <v-col col="12">
            <div class="d-flex justify-center mb-3">
              <div>
                ...
                <div>Psychics inactive {{ counts.cont1}}</div>
                <div>Psychics active {{ counts.cont2}}</div>
                <div>Clients inactive {{ counts.cont3}}</div>
                <div>Clients active {{ counts.cont4}}</div>
                <div>Psychics hidden {{ cont5 }}</div>
                <div>Clients hidden {{ cont6 }}</div>
                <div>Psychics fraud {{ cont7 }}</div>
    ...
    signup() {
      ...
      this.$store.dispatch(this.store + "/getItems", interval).then(() => {
        ...
        for (let i = 1; i <= loop; i++) {
          ...
          item = this.items.psychics.filter(item => item.interval == i);
      ...
      if (!this.year) {
        this.$store.dispatch(this.store + "/getUserStatus").then(() => {
          ...
          this.datacollection2 = {
            ...
            datasets: [
              {
                label: "Psychics inactive",
                backgroundColor: "#f87979",
                data: [this.items.psychics_inactive]
              },
              {
                label: "Psychics active",
                backgroundColor: "#664577",
