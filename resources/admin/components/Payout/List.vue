  ...
  <div>
    <div class="row">
      ...
      <div class="col-12 col-md-4">
        <v-select
          :items="[
              ...
              {text: `Pending ($${payoutsPendingAmount})`, value: 'Pending'}]"
      ...
      <div class="col-12 col-md-4 d-flex justify-end">
       ...
       <v-checkbox
              v-model="with_no_deposit_account"
              label="No Deposit Account"
              color="indigo"              
              hide-details
            ></v-checkbox>
      </div>
  ...
  methods: {
    ...
    payoutInit(option) {
      ...
      this.$store
        ...
        .then(() => {
          this.getPayoutsPendingTotalAmount()
    ...
    getQueryParam() {
      return {
        ...
        status: this.status,
        with_no_deposit_account: this.with_no_deposit_account
    ...
    getPayoutsPaidInfo(item, status, user_selected) {
      ...
      axios
        ...
        getPayoutsPendingTotalAmount() {
       let type = 'pendingTotalAmount'
       let with_no_deposit_account = this.with_no_deposit_account ? '&with_no_deposit_account='+ this.with_no_deposit_account : ''
      axios
        .get(
          `/api/admin/psychic/payouts/pending?type=${type}${with_no_deposit_account}`)
        .then(response => {
          console.log(response);
          this.payoutsPendingAmount = response.data.pendingTotalAmount;       
