<template>
  <div>
      ...
      <div class="col-12 pt-0">
        <v-radio-group v-model="filter" row class="mt-0">
          <v-radio
            ...
            v-for="(item, index) in [
              { text: 'All', value: 'all' },
              { text: 'No validate', value: 'no-validate' },
            ]"
            :key="index"
            :label="item.text"
            :value="item.value"
      ...
      <template v-slot:top>
        <v-row>
          <v-col class="d-flex justify-end">
            <v-btn icon :disabled="!role || role == 3" @click="downloadExcel" class="mr-4">
      ...
      <template v-slot:item="{ item }" style="width: 50px">
        <tr>
          ...
          <td style="white-space: nowrap">
            <a v-if="role == 1" v-bind:href="'/' + item.profile_link">{{
              item.display_name
            }}</a>
            <a v-else-if="role == 2" @click="finance(item)" href="#">{{
              item.display_name
            }}</a>
          ...
          <td>
            {{
              item.phone_numbers.prefix
                ? "(+" + item.phone_numbers.prefix + ")"
                : ""
            }}
            {{ item.phone_numbers.number }}
            <v-btn
              v-if="!item.phone_numbers.validated"
              x-small
              color="warning"
              @click="validatePhone(item.phone_numbers)"
              >validate</v-btn
            >
          ...
          <td v-if="role == 2 || !role">
            <div v-if="$can('edit', 'all')" style="white-space: nowrap">
              ...
              <v-icon color="teal" @click="showEditCredit(item)"
                >mdi-pencil</v-icon
              >
            </div>
            <span v-else>----</span>
    ...
    <v-skeleton-loader
      v-if="!paginate.data || loading"
      type="table-tbody"
      class="mx-auto"
    ></v-skeleton-loader>

    <v-dialog
      v-model="dialogFinance"
      fullscreen
      hide-overlay
    ...
    <v-dialog v-model="editCreditDialog" width="400">
      <v-card style="overflow: hidden">
        ...
        <v-row>
          <v-col cols="12" class="d-flex justify-center">
            <v-radio-group v-model="addingCredit" row class="mt-0" hide-details>
              ...
              <v-radio
                label="Removing"
                v-show="item && parseFloat(item.credit)"
                :value="false"
              ></v-radio>
            </v-radio-group>
          </v-col>
        ...
        <v-row v-show="addingCredit !== null && newCredit">
          <v-col v-if="item" cols="12" class="d-flex justify-center"
            >Current Credit Balance ${{ item.credit }}</v-col
          >
          <v-col cols="12" class="d-flex justify-center"
            >New Credit Balance ${{ newCredit }}</v-col
          >
        </v-row>

        <v-row v-show="addingCredit !== null && newCredit" class="mb-3">
          <v-col cols="12" class="d-flex justify-center">
            <v-btn small @click="editCreditDialog = false" class="mr-3"
              >Cancel</v-btn
            >
  ...
  methods: {
    validatePhone(phone) {
      const message = `Are you sure you want ${
        phone.validated ? "invalidate" : "validate"
      } the phone number?`;
      if (confirm(message)) {
        axios
          .post("/api/v1/user/verify/phone/validate/" + phone.id)
          .then((response) => {
    ...
    showEditCredit(item) {
      ...
      if (parseFloat(item.credit) == 0) this.addingCredit = true;
