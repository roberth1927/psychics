<template>
  <div class="mx-1">
    <v-menu 
      ...
      left
      z-index="301"
      content-class="tokbox-notifications my-menu"
    ...
    >
      <template v-slot:activator="{ on }">
        <a v-on="on" v-if="!mobile" style="position: relative;">
         ...
         <v-btn text icon v-else :href='get_full_url("/dashboard/notifications")'>
                  <img :src='get_full_url("icon_notification")' />
          </v-btn>
      </template>

      <v-card width="375" style="border-radius: 5px !important;" class="">
        <v-list class="p-0" style="background-color: #F0F0F7;" :style="[all_notifications.length > 4 ? {'overflow-y': 'auto','height': '77vh'} :{}]">
          <v-list-item class="py-0 " dense>
            <v-list-item-title class="header_notifications mb-4">Notifications</v-list-item-title>
             ...
             <v-list-item class="px-5 " v-if="!all_notifications.length">               
                ...
                <div style="min-height: 120px;background-color: #F4F4F4" class="mb-4 shadow">
          ...
          <v-list-item class="px-5"   v-for="(notification, index) in all_notifications" :key="notification.id">               
                ...
                <div  style="min-height: 120px;" :style="[notification.color == 'color_blue'?{'background-color': '#EBF4FD'}:{'background-color': 'white'}]" class="mb-4 shadow">
                       ...
                       <v-list-item-content style="padding-top: 15px !important;padding-bottom: 5px;">
                              ...
                              <v-list-item-title style="font-size: 15px;font-weight: 600;color: #1F1E34 !important;"> 
                                  {{show_name(notification.from)}}
                              </v-list-item-title>
                              <v-list-item-subtitle class="mt-1" style="font-size: 12px;color: #656B72 !important;">
                              {{notification.action_description}}
                            </v-list-item-subtitle>

                            <v-list-item-subtitle v-if="notification.action_description_2" class="mt-1" style="font-size: 12px;color: #656B72 !important;">
                              {{notification.action_description_2}}
                            </v-list-item-subtitle>

                              <v-list-item-subtitle v-if="notification.request_description" class="mt-1 font-weight-medium" style="font-size: 12px;color: #656B72 !important;">
        ...
        <v-card-text style="padding: 30px 20px;">
          ...
          <div class="text-center">
            ...
            <p>
              ...
              <a
                href="mailto:support@psychics1on1.com"
     ...
     checkNotification(notification,button_clicked){
        ...
        switch(notification.action){
          ...
          case 'to_psychic_online_rules':
            ...
            case 'to_online_announcement_cosmic':
                this.remove_notification(notification);                
                   setTimeout(x => {                   
                      window.open(notification.link,'_self');                   
                  }, 300); 

             case 'to_online_announcement_cosmic_refer':
                this.remove_notification(notification);                
                   setTimeout(x => {                   
                      window.open(notification.link,'_self');                   
                  }, 300); 
            
            case 'to_online_announcement_cosmic_client':
                this.remove_notification(notification);                
                   setTimeout(x => {                   
                      // window.open(notification.link);   
            ...
            case 'to_online_update_payout_method':
                this.remove_notification(notification);                
                   setTimeout(x => {                   
                      window.open(notification.link,'_self');                   
                  }, 300); 
                  ...
                  if(this.all_notifications.length > 0){
                    if(this.menu_opened){
                       ...
                       return  window.location.origin+"/images/icons/notification-current-on.svg"
                        ...
                        return  window.location.origin+"/images/icons/w-notification-on.svg"
